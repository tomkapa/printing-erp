...
mod rls_tests {
    ...
    async fn auth_tokens_migration_is_reversible(_opts: PgPoolOptions, conn: PgConnectOptions) {
        // Reverting *to* the users-RLS version undoes only the auth_tokens
        // migration. Assert the tables exist + are FORCE-protected, revert,
        // assert they are gone, re-apply, assert they are back (CLAUDE.md §13).
        let mut admin = PgConnection::connect_with(&conn)
            .await
            .expect("admin connection");
    ...
    /// Version of the users-RLS migration; reverting *to* it undoes auth_tokens.
