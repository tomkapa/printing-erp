...
/// JWT `role` claim. Authorization on roles is a separate concern (RBAC, #13);
/// here the role is only carried through tokens.
...
#[cfg(test)]
mod tests {
    use super::{DomainError, Email, Role, TenantSlug};
